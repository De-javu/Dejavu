    ...
    <div class="flex items-center justify-between p-2 group">
        ...
        <div>
            ...
            <flux:dropdown>
                ...
                <flux:menu>
                     {{-- Boton entrar --}}
                     <flux:modal.trigger name="entrar-series{{$hijas->id}}"
                     onclick="window.location.href='{{ route('series_documentales.show',$hijas->id) }}'">
                    ...
                    {{-- Boton editar --}}
                    <flux:modal.trigger name="editar-serie-{{$hijas->id}}">
                    <flux:menu.item icon="pencil-square" kbd="⌘S">
                     ...
                     {{-- Boton eliminar--}}
                     <flux:modal.trigger name="delete-serie-{{$hijas->id}}">
            ...
            <form action="{{ route('series_documentales.destroy', $hijas->id) }}" method="POST">
                @csrf
                @method('DELETE')
               <flux:modal name="delete-serie-{{$hijas->id}}"  class="min-w-[22rem]">
                 <div class="space-y-6">
                    <div>
